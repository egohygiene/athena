...
id: athena-architecture
title: Athena Architecture
...
Athena uses a layered, contract-driven architecture. This document owns structural boundaries, dependency direction, integration rules, and current-to-target evolution. Logical responsibilities remain canonical in [SYSTEM.md](SYSTEM.md).
...
```mermaid
flowchart LR
  S1[Collection registry]
  S2[Asset metadata]
  S3[Ingestion review]
  S4[License validation]
  S5[Storage layout]
  S6[Search and index projection]
  S7[Consumer export]
  S1 --> S2
  S2 --> S3
  S3 --> S4
